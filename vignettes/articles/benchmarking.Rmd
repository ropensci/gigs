...
```{r discrepancies_who_gs, echo = TRUE, eval = FALSE}
...
knitr::kable(discrepancies_who_gs, align = "ccccc")
```
...
```{r discrepancies_ig_nbs_cache, eval = FALSE, echo = FALSE}
discrepancies_ig_nbs <- waldo::compare(gigs, gs / 100, x_arg = "gigs", y_arg = "growthstandards")
```


```{r discrepancies_ig_nbs_waldo_output, eval = TRUE, echo = FALSE}
discrepancies_ig_nbs
```
...
save(file = file.path("vignettes/articles/benchmarking.rda"),
     ...
     key_pkg_label, discrepancies_who_gs, discrepancies_ig_nbs)
```
